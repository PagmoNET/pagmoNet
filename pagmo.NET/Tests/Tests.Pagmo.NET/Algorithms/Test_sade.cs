...
namespace Tests.Pagmo.NET.Algorithms
{
    ...
    public class Test_sade : TestAlgorithmBase
    {
        ...
        {
            using var problem = new TwoDimensionalSingleObjectiveProblemWrapper();
            using var algorithm = new pagmo.sade(10u);
            ...
            Assert.That(typedLines.Count, Is.GreaterThan(0), "sade verbosity should produce at least one log line");
            ...
            Assert.That(genericLines[0].AlgorithmName, Is.EqualTo("sade"));
            ...
            Assert.That(raw.ContainsKey("f"), Is.True);
            Assert.That(raw.ContainsKey("cr"), Is.True);
