...
namespace Tests.Pagmo.NET.Algorithms
{
    ...
    public class Test_pso : TestAlgorithmBase
    {
        ...
        {
            using var problem = new TwoDimensionalSingleObjectiveProblemWrapper();
            using var algorithm = new pagmo.pso(10u);
            ...
            Assert.That(typedLines.Count, Is.GreaterThan(0), "pso verbosity should produce at least one log line");
            ...
            Assert.That(genericLines[0].AlgorithmName, Is.EqualTo("pso"));
            ...
            Assert.That(raw.ContainsKey("inertia"), Is.True);
