...
namespace pagmo
{
    ...
    public partial class archipelago
    {
        ...
        public IReadOnlyList<MigrationEntry> MigrationLog
        {
            get
            {
                var count = get_migration_log_entry_count();
                var list = new List<MigrationEntry>(count);
                for (var i = 0; i < count; ++i)
                {
                    list.Add(get_migration_log_entry(i));
                }
        ...
        public IReadOnlyList<IndividualsGroup> MigrantsDb
        {
            get
            {
                var count = get_migrants_db_count();
                var list = new List<IndividualsGroup>(count);
                for (var i = 0; i < count; ++i)
                {
                    list.Add(get_migrants_db_item(i));
                }
