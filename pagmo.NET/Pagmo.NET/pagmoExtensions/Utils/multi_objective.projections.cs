...
public partial class pagmo
{
    ...
    public static ulong[] NonDominatedFront2DIndices(VectorOfVectorOfDoubles fitnessValues)
    {
        ...
        var result = new ulong[indices.Count];
        for (int i = 0; i < result.Length; i++) result[i] = (ulong)indices[i];
        return result;
    }
    ...
    {
        using var indices = select_best_N_mo(RequireNotNull(fitnessValues, nameof(fitnessValues)), (uint)selectionCount);
        var result = new ulong[indices.Count];
        for (int i = 0; i < result.Length; i++) result[i] = (ulong)indices[i];
        return result;
    }
