package io.github.samthegliderpilot.pagmonet4j.utils;
...
class MultiObjectiveTest {
    ...
    void fastNonDominatedSortingReturnsExpectedFrontShape() {
        ...
        FNDSResult result = pagmonet4j.FastNonDominatedSorting(pts);
    ...
    void paretoDominanceIsCorrect() {
        ...
        assertTrue(pagmonet4j.pareto_dominance(lhs, rhs),  "lhs dominates rhs");
        assertFalse(pagmonet4j.pareto_dominance(rhs, lhs), "rhs does not dominate lhs");
        assertFalse(pagmonet4j.pareto_dominance(lhs, lhs), "identical points do not dominate each other");
    ...
    void crowdingDistanceReturnsOneValuePerPoint() {
        ...
        DoubleVector dist = pagmonet4j.crowding_distance(pts);
