package io.github.samthegliderpilot.pagmonet4j.ext
...
class TopologyBfeAndPolicyExtensionsTest {
    ...
    @Test
    fun withTopologyUnconnectedIslandsEvolveIndependently() {
        // RED/GREEN: unconnected was missing from the DSL — adding withTopology(unconnected())
        // ensures it compiles and produces a working archipelago with no migration.
        val archi = buildArchipelago {
            withTopology(unconnected())
            val prob = SphereProblem()
            repeat(3) { i -> pushBackIsland(de(10L), prob, 16L, i.toLong()) }
        }
        archi.use {
            it.run(2L)
            // All 3 islands evolved independently; archipelago has correct size.
            assertEquals(3L, it.size())
        }
    }
