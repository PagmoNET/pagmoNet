...
%extend gaco {
    ...
    ::pagmoWrap::GacoLogEntry get_log_entry(int idx) const {
        const auto& line = $self->get_log().at((std::size_t)idx);
        return {std::get<0>(line), static_cast<unsigned long long>(std::get<1>(line)), std::get<2>(line), std::get<3>(line), std::get<4>(line), std::get<5>(line), std::get<6>(line)};
    }
